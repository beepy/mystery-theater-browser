...
// function deferredSetHeight() {
//   console.log('deferredSetHeight')
//   nextTick(() => setHeight())
// }

/**
 * NRL: removed all transitions, and height is still not correct onMounted
 * @param {[type]} () => {  setHeight()  window.addEventListener('resize', setHeight)  container.value?.addEventListener('transitioncancel', deferredSetHeight);  } [description]
 */
onMounted(() => {
  window.addEventListener('resize', setHeight);
  // container.value?.addEventListener('transitionend', deferredSetHeight);
  // container.value?.addEventListener('transitionrun', runLog);
  setHeight();
});

onBeforeUnmount(() => {
  window.removeEventListener('resize', setHeight);
  // container.value?.removeEventListener('transitionend', deferredSetHeight)
  // container.value?.removeEventListener('transitionrun', runLog);
});

defineExpose({ container, height });

// const episode = computed(() => props.episodes?.[0])
</script>
